...
break;
	...
	/*AVExe - Start*/
	mb->isTransactional = 0;
	ClassBlock* cb = CLASS_CB(mb->class);
	int canSearch = 1;
	if(strchr(cb->name, '/') && strlen(cb->name) >= 3){
		char gnuName[4] = {0};
		gnuName[3] = '\0';
		strncpy(gnuName, cb->name, 3);
		if(!strcmp(gnuName, "gnu")){
			canSearch = 0;
		}
		else if(!strcmp(gnuName, "sun")){
			canSearch = 0;
		}
		else{
			if(strlen(cb->name) >= 4){
	...
	if(mb->annotations != NULL) {
		if(canSearch){
			...
			u4 annotationsLength = mb->annotations->annotations->len;

			u2 numberAnnotations = (mb->annotations->annotations->data[0] << 8) + (mb->annotations->annotations->data[1]);

			u4 iter;
			char* annotationName = NULL;

			for(iter = 2; iter < annotationsLength; iter += 4 ){
				u2 annotationNameIdx = (mb->annotations->annotations->data[iter] << 8) + (mb->annotations->annotations->data[iter+1]) ;
				annotationName = CP_UTF8(cp,annotationNameIdx);
				if(!strcmp(annotationName, SYMBOL(avestm_Atomic))){
					mb->isTransactional = 1;
					break;
				}
			}
		}
