...
/*AVExe - Start*/
static unsigned long invokes_tx_ctr = 0;
/*AVExe - End*/
...
uintptr_t *executeJava() {
   ...
   /* Caching is supported in all variants and
	...
	/*AVExe - Start*/
    ee->last_frame->saved_transactional_depth = 0;
    if(mb->isLibrary && ee->transactional) {
    	ee->last_frame->saved_transactional_depth = ee->transactional;
    	ee->transactional = 0;
    	ee->tx_memoizer++;
    } else if(mb->isTransactional){
    	ee->transactional++;
    	stm_tx_attr_t _a = {invokes_tx_ctr++, 0};
    	sigjmp_buf *_e = stm_start(&_a);
    	if (_e != NULL){
    		if(sigsetjmp(*_e, 0)){
    			while(1){
    				MethodBlock* curMB = frame->mb;
    				if(curMB->isTransactional && ee->transactional == 1){
    					break;
    				}
    				if(curMB->isTransactional){
    					ee->transactional--;
    				}
    		        if(curMB->access_flags & ACC_SYNCHRONIZED) {
    		            Object *sync_ob = curMB->access_flags & ACC_STATIC ?
    		                    (Object*)curMB->class : (Object*)frame->lvars[0];
...
#define FIELD_ACCESS_OPCODES(level, type, suffix)          \
   ...
   DEF_OPC(OPC_GETSTATIC_QUICK##suffix, level,            \
	   PUSH_##level((ee->transactional ?															\
						(sizeof(type) == 4 ? 														\
								(type)LOAD_UINT(RESOLVED_FIELD(pc)->u.static_value.data) :			\
								(type)LOAD_UINTPTR_T(RESOLVED_FIELD(pc)->u.static_value.data)) :	\
						*(type*)(RESOLVED_FIELD(pc)->u.static_value.data)), 3);  					\
   )                                                      											\
                                                           \
    DEF_OPC(OPC_PUTSTATIC_QUICK##suffix, level,            \
        POP_STM_0((type*)(RESOLVED_FIELD(pc)->u.static_value.data), type, 3);  \
    )                                                      \
                                                           \
    DEF_OPC(OPC_GETFIELD_THIS##suffix, level,              \
    	PUSH_##level((ee->transactional ?					\
    					(sizeof(type) == 4 ? 													\
    							LOAD_UINT(&((char*)this)[GETFIELD_THIS_OFFSET(pc)]) :			\
    							LOAD_UINTPTR_T(&((char*)this)[GETFIELD_THIS_OFFSET(pc)])) :		\
    					INST_DATA(this, type, GETFIELD_THIS_OFFSET(pc))), 4);					\
    )                                                      \
                                                           \
...
/*AVExe - Start*/
#define POP_STM_0(dest, type, ins_len)						\
	if(ee->transactional){									\
		if(sizeof(type) == 4)								\
			STORE_UINT(dest, *--ostack);						\
		else												\
    		STORE_UINTPTR_T(dest, *--ostack);				\
	}														\
    else													\
    	*dest = *--ostack;									\
    DISPATCH(0, ins_len);									\
...
{                                                          \
    ...
    PUSH_0((ee->transactional ? 							\
    		(sizeof(type) == 4 ?							\
    			LOAD_UINT(&((char*)obj)[offset]) :			\
    			LOAD_UINTPTR_T(&((char*)obj)[offset]) )	 :	\
    		INST_DATA(obj, type, offset) ), 3);				\
}
/*AVExe - End*/
...
/*AVExe - Start*/
#define POP_LONG_STM(dest, ins_len)						\
	if(ee->transactional)								\
		STORE_U8(&dest, STACK_POP(uint64_t));			\
    else												\
    	dest = STACK_POP(uint64_t); 					\
    DISPATCH(0, ins_len);								\
/*AVExe - End*/
...
{                                              \
    ...
    if(ee->transactional){ PUSH_0(LOAD_INT(&(ARRAY_DATA(array, TYPE)[idx])), 1); } \
    else{ PUSH_0(ARRAY_DATA(array, TYPE)[idx], 1); }   \
}

#define ARRAY_LOAD_UINTPTR_T(TYPE)                       \
{                                              \
    int idx = ARRAY_LOAD_IDX;                  \
    Object *array = (Object *)ARRAY_LOAD_ARY;  \
                                               \
    NULL_POINTER_CHECK(array);                 \
    ARRAY_BOUNDS_CHECK(array, idx);            \
    if(ee->transactional){ PUSH_0(LOAD_UINTPTR_T(&(ARRAY_DATA(array, TYPE)[idx])), 1); } \
    else{ PUSH_0(ARRAY_DATA(array, TYPE)[idx], 1); }   \
}

#define ARRAY_LOAD_SIGNED_CHAR(TYPE)                       \
{                                              \
    int idx = ARRAY_LOAD_IDX;                  \
    Object *array = (Object *)ARRAY_LOAD_ARY;  \
                                               \
    NULL_POINTER_CHECK(array);                 \
    ARRAY_BOUNDS_CHECK(array, idx);            \
    if(ee->transactional){ PUSH_0(LOAD_SIGNED_CHAR(&(ARRAY_DATA(array, TYPE)[idx])), 1); } \
    else{ PUSH_0(ARRAY_DATA(array, TYPE)[idx], 1); }   \
}

#define ARRAY_LOAD_UNSIGNED_SHORT(TYPE)                       \
{                                              \
    int idx = ARRAY_LOAD_IDX;                  \
    Object *array = (Object *)ARRAY_LOAD_ARY;  \
                                               \
    NULL_POINTER_CHECK(array);                 \
    ARRAY_BOUNDS_CHECK(array, idx);            \
    if(ee->transactional){ PUSH_0(LOAD_UNSIGNED_SHORT(&(ARRAY_DATA(array, TYPE)[idx])), 1); } \
    else{ PUSH_0(ARRAY_DATA(array, TYPE)[idx], 1); }   \
}

#define ARRAY_LOAD_SHORT(TYPE)                       \
{                                              \
    int idx = ARRAY_LOAD_IDX;                  \
    Object *array = (Object *)ARRAY_LOAD_ARY;  \
                                               \
    NULL_POINTER_CHECK(array);                 \
    ARRAY_BOUNDS_CHECK(array, idx);            \
    if(ee->transactional){ PUSH_0(LOAD_SHORT(&(ARRAY_DATA(array, TYPE)[idx])), 1); } \
    else{ PUSH_0(ARRAY_DATA(array, TYPE)[idx], 1); }   \
}
    ...
    DEF_OPC_012(OPC_AALOAD,
        ARRAY_LOAD_UINTPTR_T(uintptr_t)
    )

    DEF_OPC_012(OPC_BALOAD,
        ARRAY_LOAD_SIGNED_CHAR(signed char)
    )

    DEF_OPC_012(OPC_CALOAD,
        ARRAY_LOAD_UNSIGNED_SHORT(unsigned short)
    )

    DEF_OPC_012(OPC_SALOAD,
        ARRAY_LOAD_SHORT(short)
    )
    ...
    DEF_OPC_012_2(
        ...
        if(ee->transactional){ PUSH_LONG(LOAD_U8(&(ARRAY_DATA(array, u8)[idx])), 1); } \
        else{ PUSH_LONG(ARRAY_DATA(array, u8)[idx], 1); }   \
    })
/*AVExe - End*/
...
{                                             \
    ...
    NULL_POINTER_CHECK(array);                \
    ARRAY_BOUNDS_CHECK(array, idx);			  \
    if(ee->transactional){ STORE_INT(&(ARRAY_DATA(array, TYPE)[idx]),val); }\
    else{ ARRAY_DATA(array, TYPE)[idx] = val; }    \
    DISPATCH(0, 1);                           \
}

#define ARRAY_STORE_CHAR(TYPE)                     \
{                                             \
    int val = ARRAY_STORE_VAL;                \
    int idx = ARRAY_STORE_IDX;                \
    Object *array = (Object *)*--ostack;      \
    ...
    NULL_POINTER_CHECK(array);                \
    ARRAY_BOUNDS_CHECK(array, idx);			  \
    if(ee->transactional){ STORE_CHAR(&(ARRAY_DATA(array, TYPE)[idx]),val); }\
    else{ ARRAY_DATA(array, TYPE)[idx] = val; }    \
    DISPATCH(0, 1);                           \
}

#define ARRAY_STORE_SHORT(TYPE)                     \
{                                             \
    int val = ARRAY_STORE_VAL;                \
    int idx = ARRAY_STORE_IDX;                \
    Object *array = (Object *)*--ostack;      \
    ...
    NULL_POINTER_CHECK(array);                \
    ARRAY_BOUNDS_CHECK(array, idx);			  \
    if(ee->transactional){ STORE_SHORT(&(ARRAY_DATA(array, TYPE)[idx]),val); }\
    else{ ARRAY_DATA(array, TYPE)[idx] = val; }    \
    DISPATCH(0, 1);                           \
}
    ...
    DEF_OPC_012(OPC_BASTORE,
        ARRAY_STORE_CHAR(char);
    )

    DEF_OPC_012_2(
            OPC_CASTORE,
            OPC_SASTORE,
        ARRAY_STORE_SHORT(short);
    )
    ...
    DEF_OPC_012(OPC_AASTORE, { 
        ...
        if(ee->transactional){ STORE_UINTPTR_T(&(ARRAY_DATA(array, Object*)[idx]),obj); }
        else{ ARRAY_DATA(array, Object*)[idx] = obj; }
        DISPATCH(0, 1);
    })
/*AVExe - End*/
    ...
    DEF_OPC_012_2(
        ...
        ARRAY_BOUNDS_CHECK(array, idx);
/*AVExe - Start*/
        if(ee->transactional){ STORE_U8(&(ARRAY_DATA(array, u8)[idx]), *(u8*)&ostack[2]); }
        else{ ARRAY_DATA(array, u8)[idx] = *(u8*)&ostack[2]; }
/*AVExe - End*/
        DISPATCH(0, 1);
    })
#endif
...
/*AVExe - Start*/
    DEF_OPC_210(OPC_MONITORENTER, {
        ...
        objectLock(obj);
        if(ee->transactional){
			LockedObject *lockedObj = sysMalloc(sizeof(LockedObject));
			lockedObj->locked = obj;
			lockedObj->next = ee->locked_list;
			ee->locked_list = lockedObj;
        }
        DISPATCH(0, 1);
    })
    ...
    DEF_OPC_210(OPC_MONITOREXIT, {
		...
		objectUnlock(obj);
        if(ee->transactional){
			LockedObject *unlockedObj = ee->locked_list;
			ee->locked_list = unlockedObj->next;
			free(unlockedObj);
        }
        DISPATCH(0, 1);
    })    
...
/*AVExe - Start*/
    DEF_OPC_210(OPC_GETSTATIC2_QUICK, {
        FieldBlock *fb = RESOLVED_FIELD(pc);
        //o fb->u.static_value.l já tem o long long carregado
        PUSH_LONG((ee->transactional ?
        				LOAD_U8(&(fb->u.static_value.l)) :
        				fb->u.static_value.l), 3);
    })

    DEF_OPC_012(OPC_PUTSTATIC2_QUICK, {
        FieldBlock *fb = RESOLVED_FIELD(pc);
        //o fb->u.static_value.l é o próprio long long...
        POP_LONG_STM(fb->u.static_value.l, 3);
    })
    ...
    DEF_OPC_210(OPC_GETFIELD2_QUICK, {
        ...
        PUSH_LONG((ee->transactional ?
        		LOAD_U8(&((char*)obj)[SINGLE_INDEX(pc)]) :
        		INST_DATA(obj, u8, SINGLE_INDEX(pc))), 3);
    })
/*AVExe - End*/
...
#define PUTFIELD_QUICK(type, suffix)                         \
    ...
    DEF_OPC_012(OPC_PUTFIELD2_QUICK, {
        ...
        /*AVExe - Start*/
        if(ee->transactional)
        	STORE_U8(&INST_DATA(obj, u8, SINGLE_INDEX(pc)), *(u8*)&ostack[1]);
        else
...
#define PUTFIELD_QUICK(type, suffix)                        \
    DEF_OPC_012(OPC_PUTFIELD_QUICK##suffix, {               \
        ...
        if(ee->transactional){																\
        	if(sizeof(type) == 4)															\
        		STORE_UINT(&INST_DATA(obj, type, SINGLE_INDEX(pc)), ostack[1]); 			\
        	else																			\
        		STORE_UINTPTR_T(&INST_DATA(obj, type, SINGLE_INDEX(pc)), ostack[1]); 		\
        }																					\
        else 																				\
...
invokeMethod:
{
    ...
    if(new_mb->access_flags & ACC_NATIVE) {
        ...
        if(mb->isLibrary && ee->transactional) {
        	...
        	if (_e != NULL){
        		if(sigsetjmp(*_e, 0)){
        			/* Transaction restarted. Climbing stack frames until reaching correct one for retry */
        			while(1){
        				MethodBlock* curMB = frame->mb;
        				if(curMB->isTransactional && ee->transactional == 1){
        					break;
...
methodReturn:
	/*AVExe - Start*/
	if(mb->isTransactional){
		COMMIT
		ee->transactional--;
		CHECK_FOR_PENDING_GC
	}

    if(frame->saved_transactional_depth) {
    	ee->transactional = frame->saved_transactional_depth;
    	frame->saved_transactional_depth = 0;
    	ee->tx_memoizer--;
    }
	/*AVExe - End*/
