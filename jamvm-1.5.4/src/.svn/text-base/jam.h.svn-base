...
typedef struct locked_object {
	Object *locked;
	struct locked_object *next;
} LockedObject;
...
typedef struct exec_env {
    ...
    char overflow;
    /*AVExe - Start*/
    LockedObject *locked_list;	//keeps the objects locked with monitor enter but not yet unlocked. last one is always at head
    volatile int transactional;	//transactional depth
    volatile int tx_memoizer;	//memoizer of transactional depth while inside code called from JNI
    volatile int allow_gc;		//flag acknowledging GC that although we are in transactional context, we are waiting in a safe point
    /*AVExe - End*/
} ExecEnv;
...
#define CHECK_FOR_PENDING_GC															\
	/* Check if GC wants to run */														\
	if(gcIntent()){																		\
		ee->allow_gc = 1;																\
		if(checkIfLastTxToAck()){														\
			gc1();																		\
		}																				\
		else {																			\
			suspendSelf();															\
...
#define UNLOCK_MONITORS											\
	while(ee->locked_list != NULL){								\
		LockedObject* objToUnlock = ee->locked_list;			\
		objectUnlock(objToUnlock->locked);						\
		ee->locked_list = objToUnlock->next;					\
		free(objToUnlock);										\
    }
...
#define ABORT							stm_get_attributes()->no_retry = 1; stm_abort(0);
#define ABORT_AND_RETRY					stm_abort(0);

#define LOAD_SIGNED_CHAR(addr)			stm_load_uchar((volatile unsigned char *)addr)
#define LOAD_UNSIGNED_SHORT(addr)		stm_load_ushort((volatile unsigned short *)addr)
#define LOAD_SHORT(addr)				stm_load_short((volatile short*)addr)
#define LOAD_UINT(addr)					stm_load_uint((volatile unsigned int *)addr)
#define LOAD_INT(addr)					stm_load_int((volatile int *)addr)
#define LOAD_UINTPTR_T(addr)			stm_load((stm_word_t *)addr)	//stm_word_t = uintptr_t
#define LOAD_U8(addr)					(u8)stm_load_u64((volatile uint64_t *)addr)	//u8 = unsigned long long, jamvm 'hopes' that is 64bits

#define STORE_CHAR(addr, value)			stm_store_char((volatile char *)addr, (char)value)
#define STORE_SHORT(addr, value)		stm_store_short((volatile short *)addr, (short)value)
#define STORE_UINT(addr, value)			stm_store_uint((volatile unsigned int *)addr, (unsigned int)value)
#define STORE_INT(addr, value)			stm_store_int((volatile int *)addr, (int)value)
#define STORE_UINTPTR_T(addr, value)	stm_store((stm_word_t *)addr, (stm_word_t)value)	//stm_word_t = uintptr_t
#define STORE_U8(addr, value)			stm_store_u64((volatile uint64_t *)addr, (uint64_t)value) //u8 = unsigned long long, jamvm 'hopes' that is 64bits
...
extern int anyTransactionOcurring();
extern int checkIfLastTxToAck();
