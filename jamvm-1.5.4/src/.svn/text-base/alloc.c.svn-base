...
/*AVExe - Start*/
/* flag that sinalizes intent for GC to run */
static volatile int gc_intent = 0;
       ...
       tries = 0;
        ...
        switch(state) {
            ...
            case run_finalizers:
                ...
                /*AVExe - Start*/
                //check GC is being inhibited due to transactions. if so, we abort ourselves with retry here
                //we could alternatively do a GC and jumping marked 'pinned' objects due to transactions
                if(self->ee->transactional){
                    unlockVMLock(heap_lock, self);
                    ABORT_AND_RETRY
                }
                /*AVExe - End*/
