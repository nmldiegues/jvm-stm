...
void suspendAllThreads(Thread *self) {
    ...
    for(thread = &main_thread; thread != NULL; thread = thread->next) {
		...
		/*AVExe - Start*/
        if(!thread->blocking && !thread->ee->allow_gc) {
        /*AVExe - End*/
            TRACE("Sending suspend signal to thread 0x%x id: %d\n",
                  thread, thread->id);
            pthread_kill(thread->tid, SIGUSR1);
    ...
    for(thread = &main_thread; thread != NULL; thread = thread->next) {
	    /*AVExe - Start*/
        if(thread == self || thread->ee->allow_gc)
            continue;
        /*AVExe - End*/
...
int anyTransactionOcurring(){
	...
	for(thread = &main_thread; thread != NULL; thread = thread->next){
		...
		if(ee->transactional || ee->tx_memoizer){
			anyTx = 1;
			break;
		}
	}
...
int checkIfLastTxToAck(){
	...
	for(thread = &main_thread; thread != NULL; thread = thread->next){
		...
		ExecEnv *ee = thread->ee;
		if((ee->transactional || ee->tx_memoizer) && !ee->allow_gc){
			allAcked = 0;
			break;
		}
	}
